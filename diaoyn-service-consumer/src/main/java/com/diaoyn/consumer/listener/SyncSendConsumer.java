...
public class SyncSendConsumer implements RocketMQListener<MessageExt> {

    @Override
    public void onMessage(MessageExt message) {
        String orderId = message.getProperty(MessageConst.PROPERTY_KEYS);
        String content = message.getProperty("content");
        log.info("SyncSendConsumer延迟消息被处理了，id = {}, orderId = {}, content = {}", message.getMsgId(), orderId, content);
