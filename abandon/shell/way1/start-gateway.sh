...
start(){
  ...
  if [ $? -eq "0" ]; then
    ...
    nohup java -jar ${APP_PATH}/${APP_NAME}.jar --spring.profiles.active=${PROFILE} --spring.config.location=${YML_PATH}/demo-gateway/bootstrap.yml > ${LOG_PATH}/demo-gateway/demo-gateway.out 2>&1 &
    echo 'Start success active='${PROFILE}' LOG_PATH is '${LOG_PATH}'/demo-gateway/demo-gateway.out'
 fi
}
#引入通用部署工具
